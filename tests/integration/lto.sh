...
"$corc" compile --nostdlib --freestanding --flat tests/integration/lto/Caller.cor --ref tests/integration/lto/Value.cor --with "$work/bare-value.o" -o "$work/convenience.bin"
if "$corc" compile --nostdlib --freestanding --flat tests/integration/lto/Caller.cor --ref tests/integration/lto/Value.cor --with "$work/value.o" -o "$work/bad-flat.bin" 2> "$work/bad-flat.log"; then
