...
```
tests/lang/run.sh              # every test
tests/lang/run.sh strings      # tests whose file name contains "strings"
tests/lang/run.sh -v hello     # also show what the compiler printed
tests/lang/run.sh --opt-size  # every test using size-oriented inlining
```

The runner builds `compiler/corc.csproj` (Release) if no `corc.dll` is found,
then for each `tests/lang/*.cor` runs

```
corc compile lib/std.cor lib/rt/runtime.cor lib/rt/gc.cor lib/threading.cor lib/sys/linux.cor tests/lang/<name>.cor -o <tmp>/<name>
<tmp>/<name>
```
...
| `CORC_LIBS` | library sources compiled before the test (default `lib/std.cor lib/rt/runtime.cor lib/rt/gc.cor lib/threading.cor lib/sys/linux.cor`) |
...
- `sources` (optional) lists extra source files, relative to the repository
  root, compiled with the test: how kernel code such as `os/kernel/net/` is
  tested on Linux before it runs on the machine.
...
1. Copy the shape of an existing test: a `static class Program` with
   `static int Main()`, printing through `Sys.Print`, `Sys.PrintInt`,
   `Sys.PrintChar` and `Sys.PrintLine` (most tests define a one-line `Line`
   helper). No namespaces or usings.
...
3. Use only language features that the standard library (`lib/std.cor`,
   `lib/heap.cor`, `lib/io.cor`) or an existing test already uses, so the test
   measures the code generator rather than the parser's limits. Grep
   `compiler/Lang/Parser.cs` if unsure whether a construct is accepted.
...
5. Number the file so it sorts after related tests (`NN_name.cor`), fill in
   the header, and run `tests/lang/run.sh name` until it passes.
