...
git archive "$revision" compiler lib include tests/lang/run.sh tests/perf/selfcompile-smoke.cor | tar -x -C "$snapshot"
cd "$snapshot"
export CORC_LIB="$snapshot/lib"
read -r -a libraries <<< "$(sed -n 's/^libs="${CORC_LIBS:-\(.*\)}"/\1/p' tests/lang/run.sh)"
test "${#libraries[@]}" -gt 0
mapfile -t sources < <(rg --files compiler -g '*.cs' -g '!**/tests/**' -g '!**/bin/**' -g '!**/obj/**' -g '!**/Legacy/**' | LC_ALL=C sort)
test "${#sources[@]}" -gt 0
jobs="${SELF_JOBS:-1}"
...
measure reference-smoke-compile "$bootstrap" compile "${flags[@]}" "${libraries[@]}" tests/perf/selfcompile-smoke.cor -o "$work/smoke-reference"
measure reference-smoke-run "$work/smoke-reference"
printf 'selfcompile smoke passed\n' | cmp - "$work/reference-smoke-run.log"
measure smoke-compile "$work/stage1" compile "${flags[@]}" "${libraries[@]}" tests/perf/selfcompile-smoke.cor -o "$work/smoke"
...
test -s "$work/stage2"
measure stage2-smoke-compile "$work/stage2" compile "${flags[@]}" "${libraries[@]}" tests/perf/selfcompile-smoke.cor -o "$work/smoke2"
